...
import com.example.hackchallengenewsfrontend.networking.Article
...
fun NewsScreen(
    ...
    LazyColumn(
        ...
        item {
            ...
            if (outletFilters.size > 1) {
                FilterRow(
                    ...
                    val outletId =
                        outletFilters.entries.find { it.value == selectedFilter }?.key
                ...
                AnimatedContent(
                    ...
                    transitionSpec = {
                        if (targetState) {
                            ...
                            // Exiting loading: slide in from bottom
                            slideInVertically(
                    ...
                    label = "content_filter_animation"
                ) { isLoading ->
                    if (isLoading) {
                        Box(
                            modifier = Modifier
                                ...
                                Column() {
                                    if (!uiState.feed.isEmpty()) {
                                        ...
                                        NewsCard(
                                            ...
                                            date = firstNewsCard.date?.toHumanReadable()
                                                ?: firstNewsCard.date.toString(),
        ...
        if (uiState.feed.size > 3) {
            val popularItems =
                uiState.feed.slice((popularItemsList + 1)..(uiState.feed.size - 1))
                items(
                    items = popularItems,
